...
async function submitToFormspree(data: Record<string, unknown>, toast: ReturnType<typeof useToast>['toast']): Promise<boolean> {
  if (!FORMSPREE_ID) {
    toast({
          ...
          >
            <Star
              className={`w-7 h-7 transition-colors ${
                star <= (hovered || value) ? 'fill-accent text-accent' : 'text-gray-300'
              }`}
            />
          </button>
        ))}
        {(hovered || value) > 0 && (
          <span className="ml-2 text-sm font-semibold text-accent">{labels[hovered || value]}</span>
        )}
      </div>
    </div>
  );
...
export default function Contact() {
  ...
  const handleQuoteSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const data = { ...Object.fromEntries(new FormData(form)), _subject: 'Quote Request — Devcon Industrial Solutions' };
    try {
      await submitToFormspree(data, toast);
      form.reset();
      toast({ title: 'Message Sent!', description: "Thank you! We'll get back to you within 24 hours.", duration: 6000 });
    } catch {
      toast({ title: 'Could not send message', description: 'Please email us directly at devconsalesgoa@gmail.com', variant: 'destructive', duration: 8000 });
    } finally {
  ...
  const handleFeedbackSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (rating === 0) {
      toast({ title: 'Please select a star rating', variant: 'destructive', duration: 3000 });
      return;
    }
    setFeedbackSubmitting(true);
    const form = e.target as HTMLFormElement;
    const fields = Object.fromEntries(new FormData(form));
    const data = { ...fields, rating: `${rating}/5`, _subject: `Customer Feedback (${rating}★) — Devcon Industrial Solutions` };
    try {
      await submitToFormspree(data, toast);
      form.reset();
      setRating(0);
      toast({ title: 'Thank you for your feedback! 🙏', description: 'Your response has been recorded.', duration: 6000 });
    } catch {
      toast({ title: 'Could not submit feedback', description: 'Please email us directly at devconsalesgoa@gmail.com', variant: 'destructive', duration: 8000 });
    } finally {
          ...
          >
            <div>
              ...
              <p className="text-lg text-foreground/70 mb-8">
                Ready to optimise your plant's efficiency? Contact us for quotes, technical support, or site visits.
              </p>
          ...
          >
            ...
            <form onSubmit={handleQuoteSubmit} className="space-y-5">
              ...
              <div className="space-y-2">
                ...
                <Input id="company" name="company" placeholder="Acme Industries Pvt Ltd" className="bg-muted/50 border-border" />
              </div>
        ...
        >
          <div className="bg-white rounded-2xl shadow-lg border border-border p-8">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h3 className="text-2xl font-bold text-primary">Share Your Feedback</h3>
            </div>
            <p className="text-foreground/60 text-sm mb-6">
              Worked with us? We'd love to hear about your experience — your feedback helps us serve you better.
            </p>
            ...
            <form onSubmit={handleFeedbackSubmit} className="space-y-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input id="fb-name" name="name" placeholder="Ramesh Sharma" required className="bg-muted/50 border-border" />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="fb-company">Company (optional)</Label>
                  <Input id="fb-company" name="company" placeholder="ABC Industries" className="bg-muted/50 border-border" />
                </div>
              </div>
              ...
              <Button type="submit" className="w-full bg-accent text-primary hover:bg-accent/90 font-bold h-11" disabled={feedbackSubmitting}>
                {feedbackSubmitting ? 'Submitting…' : <><Star className="w-4 h-4 mr-2" /> Submit Feedback</>}
              </Button>
            </form>
          </div>
        </motion.div>
